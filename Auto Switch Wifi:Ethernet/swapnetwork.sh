...
# Purpose:  Disable network bridging. Allows only one network connection (ethernet or Wi-Fi) to be live 
...
##
# Define wireless interface "en" (ethernet) label.
...
##
# Sleep for a minimum of 10 seconds (per Appple recommendation--> https://developer.apple.com/library/content/documentation/MacOSX/Conceptual/BPSystemStartup/Chapters/CreatingLaunchdJobs.html) to prevent launchd interpreting as a crashed process.
sleep 10
exit 0