...
## RYQ (Capital Group Companies)


## First thing first, install CG Logos

sudo jamf policy -trigger install_icons
#########################################################################################
# Testing Mode
#########################################################################################
...
# Service branding, please see the Customized Self Service Branding area below
	BANNER_IMAGE_PATH="/Library/Application Support/CGIT/cg_logo_2048.png"

# Main heading that will be displayed under the image
# If this variable is left blank, the generic banner will appear
	BANNER_TITLE="Capital Group macOS provisioning tool"
...
# quotes below as double quotes will break the new lines.
	MAIN_TEXT='The provisioning process will take 20-30 minutes to complete. \n \n Please ensure this Mac is plugged into power.'
...
# deferred enablement is detected. Text displayed if deferred status is on.
	# Option for dropdown alert box
		FV_ALERT_TEXT="This Mac must logout to start the encryption process. You will be asked to enter your password and click OK or Continue a few times. Your Mac will be usable while encryption takes place."
...
# Text that will display inside the alert once policies have finished
	# Option for dropdown alert box
		COMPLETE_ALERT_TEXT="This Mac is now finished with initial setup and configuration. Press Quit to continue."
	# Options if not using dropdown alert box
...
# Main heading that will be displayed under the image
	ERROR_BANNER_TITLE="Error during provisioning process..."

# Paragraph text that will display under the main heading. For a new line, use \n
# If this variable is left blank, the generic message will appear. Leave single
# quotes below as double quotes will break the new lines.
	ERROR_MAIN_TEXT="Please reach out to End User Platform - Mobile and Mac team."
...
# run in order as they appear below.
	POLICY_ARRAY=(
		"Installing JAMF Connect Login,install_jamf_connect_login"
		"Enabling location services,enable_location_services"
		"Installing Chrome,install_google_chrome"
		"Installing WebEx Productivity Tools,install_webex"
		"Installing Symantec Endpoint Protection,install_sep"
		"Installing SecurID,install_rsa_securid"
		"Installing NoMAD,install_nomad"
		"Installing Cisco Jabber,install_jabber"
		"Installing PrinterLogic Print Installer,install_printerlogic"
		"Installing Microsoft Office 2016,install_office"
		"Installing Bomgar,install_bomgar"
		"Installing Cisco AnyConnect,install_anyconnect"
		"Creating CGADMIN account,create_cgadmin_account"
        "Setting dock icons,set_dock"
		"Install all available macOS updates,install_macos_updates"
	)
...
# Registration window configuration
	...
	# Registration window title
		REGISTRATION_TITLE="Set Computer Name"

	# Registration status bar text
		REGISTRATION_STATUS="Please click the button below to name computer and set Wi-Fi network..."

	# Registration window submit or finish button text
		REGISTRATION_BUTTON="Set computer name"
	...
	#######################################################################################
		...
		# Place Holder Text
			REG_TEXT_LABEL_1_PLACEHOLDER="ml or md and asset #"
		...
		# Help Bubble for Input. If title left blank, this will not appear
			...
			REG_TEXT_LABEL_1_HELP_TEXT="Enter the asset # of the machine. Be sure to include the prefix of ml or md."
  ...
  #######################################################################################
    # Label for the popup
      REG_POPUP_LABEL_1="Wi-Fi Network"
    ...
    # Help Bubble for Input. If title left blank, this will not appear
      REG_POPUP_LABEL_1_HELP_TITLE="Wi-Fi Network Dropdown Field"
      REG_POPUP_LABEL_1_HELP_TEXT="Please choose the appropriate network for the end-user. If IMAnetwork is selected, ensure user is in the correct AD group."
    ...
    # when needed later on. BE VERY CAREFUL IN CHANGING THE FUNCTION!
      REG_POPUP_LABEL_1_LOGIC (){
        ...
        if [ "$TESTING_MODE" = true ]; then
          ...
          /usr/bin/touch /private/var/db/receipts/com.cg."$REG_POPUP_LABEL_1_VALUE".bom
        fi
      }
...
# Nice completion text and run recon. Also check BOM file to see if machine is part of ILA or IMA network
	echo "Status: $INSTALL_COMPLETE_TEXT" >> "$DEP_NOTIFY_LOG"
    "$JAMF_BINARY" policy -trigger configure_network_imanetwork
    "$JAMF_BINARY" policy -trigger configure_network_ilanetwork
    "$JAMF_BINARY" recon
    
# Drop .BOM file to verify completion of provisioning
	/usr/bin/touch /private/var/db/receipts/com.cg.provisioningcomplete.bom
